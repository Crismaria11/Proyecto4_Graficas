...
uniform sampler2D tex;
uniform vec4 diffuse;
uniform vec4 change12;

void main()
{

  fragColor = diffuse + change12 * texture(tex, vertexTexcoords) * intensity;
}
"""
fragment_shader3 = """
#version 460
...
uniform sampler2D tex;
uniform vec4 change1;
uniform vec4 change12;

void main()
{

  fragColor = change1 + change12 * texture(tex, vertexTexcoords) * intensity;
}
"""

shader = compileProgram(
  compileShader(vertex_shader, GL_VERTEX_SHADER),
  compileShader(fragment_shader1, GL_FRAGMENT_SHADER)
)


scene = pyassimp.load('./sheep.obj')

texture_surface = pygame.image.load('./sheep.bmp')
...
def glize(node):
  ...
  for mesh in node.meshes:
    ...
    glUniform4f(
      glGetUniformLocation(shader, "change1"),
      0.8, 0.8, 0.8, 1
    )

    glUniform4f(
      glGetUniformLocation(shader, "change12"),
      0.8, 0.8, 0.8, 1
    )
...
def createTheMatrix(counter, camera):
  ...
  scale = glm.scale(i, glm.vec3(20, 20, 20))
  ...
  for event in pygame.event.get():
    if event.type == pygame.QUIT:
      ...
      if event.key == pygame.K_n:
        ...
        glUseProgram(shader)
        pygame.mixer.Channel(0).play(pygame.mixer.Sound('./inicio.mp3'))
      ...
      if event.key == pygame.K_m:
        ...
        pygame.mixer.Channel(0).play(pygame.mixer.Sound('./mal.mp3'))
        pygame.mixer.Channel(0).set_volume(0.3)
      ...
      if event.key == pygame.K_b:
        shader = compileProgram(
          compileShader(vertex_shader, GL_VERTEX_SHADER),
          compileShader(fragment_shader3, GL_FRAGMENT_SHADER)
        )
        glUseProgram(shader)
        camera.x -= camera_speed
        camera.z -= camera_speed
        ...
        pygame.mixer.Channel(0).play(pygame.mixer.Sound('./bien.mp3'))
        pygame.mixer.Channel(0).set_volume(0.3)
      ...
      if event.key == pygame.K_f:
        ...
        glUseProgram(shader)
        pygame.mixer.Channel(0).play(pygame.mixer.Sound('./final.mp3'))
        pygame.mixer.Channel(0).set_volume(0.3)
